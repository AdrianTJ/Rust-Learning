# Rust Learning

A dedicated space for exploring the Rust programming language, documenting progress, and storing practice exercises.

## Overview

This repository contains code snippets, small projects, and notes gathered while learning Rust.

## Prerequisites

To run the code in this repository, you'll need the Rust toolchain installed:

- **Rust & Cargo:** [Install Rust](https://www.rust-lang.org/tools/install)

## Getting Started

The `projects/` directory is a Cargo workspace, so run a specific crate by name
from the repo root:

```bash
cargo run -p guessing_game
```

`cargo check --workspace` type-checks everything at once.

## What's in here

- **[`CHECKLIST.md`](./CHECKLIST.md)** — the tracker. What to do next, in order.
  Start here.
- [`CURRICULUM.md`](./CURRICULUM.md) — why each Rust resource, and how fast to read it
- [`LEAD_AI_CURRICULUM.md`](./LEAD_AI_CURRICULUM.md) — the 12-month AI engineer track
- [`typescript/`](./typescript/) — a self-contained side quest: 3–4 weeks of
  TypeScript, ending in an agent harness. Own curriculum and checklist; not a
  prerequisite for anything above.
- [`PROJECTS.md`](./PROJECTS.md) — the 8-project build ladder
- `projects/` — the crates themselves

## Resources

- [The Rust Programming Language (The Book)](https://doc.rust-lang.org/book/)
- [Rust by Example](https://doc.rust-lang.org/stable/rust-by-example/)
- [Rust Standard Library Documentation](https://doc.rust-lang.org/std/)
